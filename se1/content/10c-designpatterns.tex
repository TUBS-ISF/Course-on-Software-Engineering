	...
	\begin{fancycolumns}
		\begin{definition}{Observer \mysource{\gofen}}
			...
			\begin{tabularx}{\textwidth}{rX}				
				...
				%example & no coupling between model and view in model-view-controller architectures\\ % MVC moved to SE2
				example & multiple diagrams (e.g., views) shall be updated whenever data in a table (e.g., model) changes\\
				idea & data object (e.g., model) has a list of observers (e.g., views) that are notified about changes
			\end{tabularx}
		\end{definition}
		\nextcolumn
