	...
	\begin{fancycolumns}[widths={55}]
		\begin{definition}{Network Diagram \deutsch{Netzplan}}
			\begin{itemize}
				...
				\item nodes represent tasks, edges represent dependencies
			\end{itemize}
		\end{definition}
		\pause
		\begin{definition}{Metra Potential Method}
			...
			\begin{itemize}
				\item \emph{earliest start} and \emph{earliest finish} time calculated with \emph{forward pass}
				\item \emph{latest start} and \emph{latest finish} time calculated with \emph{backwards pass}
				\item \emph{buffer}: time span between earliest and latest start/finish
				\item \emph{critical path}: longest path(s) of tasks without buffer
			\end{itemize}
		\end{definition}
		\nextcolumn
		\pause
