...
\begin{frame}[fragile]
    \frametitle{\insertsection{} -- \insertsubsection{}}
    \begin{example}{Example: Run Operation on All Flyable Objects}
        \begin{fancycolumns}[widths={50,50}]
            \textbf{OOP}
            \begin{lstlisting}[language=Java, basicstyle=\footnotesize,frame=single,backgroundcolor=\color{gray}]
...
for (GameObject obj : gameObjects) {
    if (obj instanceof Flyer) {
        ...
        ((Runner)runComp).run();
            ...
            \nextcolumn
            \textbf{ECS}
            \begin{lstlisting}[language=Java, basicstyle=\footnotesize,backgroundcolor=\color{gray},frame=single]
List<FlyComponent> flyComponents;
List<RunComponent> runComponents;
for (FlyComponent flyComp : flyComponents) {
    flyComp.fly();
}
for (RunComponent runComp : runComponents) {
    runComp.run();
}
    ...
    \end{example}
    \begin{fancycolumns}[widths={50,50}]
        \begin{motivation}{Advantage: No Runtime Polymorphism}
            \begin{itemize}
                \item OOP: Runtime polymorphism $\rightarrow$ Overhead of dynamic dispatch
                \item ECS: Systems operate on specific component types $\rightarrow$ No overhead caused by dynamic dispatch and case distinctions
            \end{itemize}
        \end{motivation}
        \nextcolumn
        \begin{motivation}{Advantage: Cache-Friendly Access Patterns}
            \begin{itemize}
                \item OOP: Data storage is guided by object structure $\rightarrow$ Scattered memory access, poor cache performance
                \item ECS: Components of the same type are stored contiguously $\rightarrow$ Cache locality
            \end{itemize}
        \end{motivation}
    \end{fancycolumns}
    ...
    \begin{fancycolumns}[widths={40,60}]
        \begin{motivation}{Advantages}
            \begin{itemize}
                ...
                \item Performance benefits due to data-oriented design and cache-friendly access patterns
            \end{itemize}
        \end{motivation}
