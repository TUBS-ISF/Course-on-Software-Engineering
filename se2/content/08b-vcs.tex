...
\begin{frame}[fragile]{\insertsubsection}
	\begin{definition}{Merging Two Branches}
		\begin{enumerate}
			\item Identify common ancestor commit
			\item Create merge commit that combines both histories after common ancestor
			\item Automatic merge for artifacts only touched by one history
			\item Manual merge for conflicts
		\end{enumerate}

	\end{definition}
	\begin{fancycolumns}[animation=none, widths={30}]
		\begin{lstlisting}[language=bash, basicstyle=\scriptsize, breaklines=true, numbers=none, escapechar=*]
git checkout main
...
\begin{frame}{\insertsubsection{} \mytitlesource{\href{https://gitlab.com/gitlab-org/gitlab-foss/-/blob/0fdb03ee16f0ccd7f122a4f0af23ee628d1de3c9/doc/workflow/gitlab\_flow.md}{GitLab Documentation}}}
	\begin{fancycolumns}[animation=none]
		\begin{definition}{Git Flow}
			\begin{itemize}
				\item First proposal to use branches
	...
	\begin{fancycolumns}[animation=none]
		\begin{definition}{GitHub Flow}
			\begin{itemize}
				...
				\item Extensions with deployment branches
			\end{itemize}
		\end{definition}\pause
		\begin{note}{Advantages}
			\begin{itemize}
				...
				\item Code fast in Main $\rightarrow$ minimizes "dead" code
			\end{itemize}
		\end{note}
		\nextcolumn
