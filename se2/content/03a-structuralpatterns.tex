\subsection{Design Patterns}
\slideDesignPatterns

\subsection{Gang of Four}
\slideGangOfFour

\subsection{Structural Design Patterns}
\begin{frame}[label=structuralpatterns]{Overview on Design Patterns \mytitlesource{\gof}}
	\centering\slideMindmapDesignPatterns{}{}{}{}{}{}{}{visible on={<-0|handout:0>}}{visible on={<-0|handout:0>}}
\end{frame}

\subsection{Composite Pattern}
\begin{frame}{\insertsubsection}
	\begin{fancycolumns}
		\begin{definition}{Composite \mysource{\gof}}
			\setlength\tabcolsep{1mm}
			\begin{tabularx}{\textwidth}{rX}
				intent     & \mycite{Compose objects into tree structures to represent part-whole hierarchies. Composite lets clients treat individual objects and compositions of objects uniformly.} \\
				% aka. & \\
				motivation & avoid case distinctions for primitive and compound objects                                                                                                                \\
				idea       & create a common abstract super class enabling a unified access
			\end{tabularx}
		\end{definition}
		\uncover<3->{\begin{example}{Java GUIs with Swing\mysource{\href{https://docs.oracle.com/javase/8/docs/api/javax/swing/JComponent.html}{oracle.com}}}
				\begin{itemize} % TODO graphical visualization of the different elements
					\item Component: javax.swing.JComponent
					\item Leaf: JLabel, JButton, JTextField, \ldots
					\item Composite: JPanel, JTable, JTree, \ldots
				\end{itemize}
			\end{example}}
		\nextcolumn
		\composite{width=\linewidth}
	\end{fancycolumns}
\end{frame}


\begin{frame}
	\frametitle{\insertsubsection{} -- Example}
	\begin{fancycolumns}[widths={35,65}]
		\begin{example}{Example -- Formula Tree}
			\begin{itemize}
				\item Components are formula parts
				\item Leaves are literals/numbers
				\item Operators (sum, product) are composites
				\item Formula parts can be arbitrarily nested
				\item Parents can always assume that children provide subresult in compute()
			\end{itemize}
		\end{example}
		\nextcolumn
		\begin{figure}[htbp]
			\centering

			\includegraphics[width=\columnwidth]{uml/compositeexample.pdf}
		\end{figure}
	\end{fancycolumns}


\end{frame}

\begin{frame}[fragile]
	\frametitle{\insertsubsection{} -- vs Naive}

	\begin{fancycolumns}[widths={50,50}, t]
		\centering \textbf{Composite Pattern}
		\begin{example}{Class}
			\vspace{-3mm}
			\begin{lstlisting}[language=Java, basicstyle=\small]
public class Product extends FormulaComponent { ... }

public class Literal extends FormulaComponent { ... }
			\end{lstlisting}
			\vspace{-3mm}
		\end{example}
		\begin{example}{Compute Method}
			\begin{lstlisting}[language=Java, basicstyle=\small]
@Override
public int compute() {
	int product = 1;
	for (FormulaComponent component : getChildren()) {
		product = product * component.compute();
	}
	return product;
}
			\end{lstlisting}
		\end{example}
		\nextcolumn
		\centering \textbf{Naive}
		\begin{example}{Class}
			\vspace{-3mm}
			\begin{lstlisting}[language=Java, basicstyle=\small]
public class Product { ... }

public class Literal { ... }
			\end{lstlisting}
			\vspace{-3mm}
		\end{example}
		\begin{example}{Compute Method}
			\vspace{-3mm}
			\begin{lstlisting}[language=Java, basicstyle=\small]
public int compute() {
	int product = 1;
	for (Object element : getChildren()) {
		if (element instanceof Product) {
			product = product * ((Product) element).compute();
		} else if (element instanceof Sum) {
			product = product * ((Sum) element).compute();
		} else if (element instanceof Literal) {
			product = product * ((Literal) element).value;
		}
	}
	return product;
}
		\end{lstlisting}
			\vspace{-3mm}
		\end{example}
	\end{fancycolumns}
\end{frame}

\subsection{Object Adapter Pattern}
\begin{frame}{\insertsubsection} % TODO copied from SE1. reuse?
	\begin{fancycolumns}
		\begin{definition}{Object Adapter \mysource{\gof}}
			\setlength\tabcolsep{1mm}
			\begin{tabularx}{\textwidth}{rX}
				intent     & \mycite{Convert the interface of a class into another interface clients expect. Adapter lets classes work together that couldn't otherwise because of incompatible interfaces.} \\
				aka.       & wrapper                                                                                                                                                                         \\
				motivation & enable reuse of classes even though incompatible interfaces cannot be made compatible                                                                                           \\
				idea       & create a new class with a compatible interface that forwards all requests
			\end{tabularx}
		\end{definition}
		\nextcolumn
		\objectadapter{width=\linewidth}
	\end{fancycolumns}
\end{frame}

\begin{frame}{\insertsubsection{} -- Example}
	\begin{fancycolumns}[widths={45,55}]
		\begin{example}{Example -- Orders as Formula Components}
			\begin{itemize}
				\item Goal: Use orders in calculations
				\item With adapter pattern, we can treat orders as formula components without changing their implementation
				\item Problems with alternatives
				\begin{itemize}
					\item Make order implement formula component: Confusing, required to adapt (possibly external) class, violates single responsibility principle
					\item External utility functions for conversion: Considerable overhead when using orders especially for larger interfaces, error-prone
				\end{itemize}
			\end{itemize}
		\end{example}
		\nextcolumn
		\begin{figure}[htbp]
			\centering

			\includegraphics[width=\columnwidth]{uml/adapterexample.pdf}
		\end{figure}
	\end{fancycolumns}
\end{frame}

% \begin{frame}{Example of the Decorator Pattern \mytitlesource{\href{https://docs.oracle.com/en/java/javase/21/docs/api/java.base/java/io/FilterInputStream.html}{Java 21}}}
% 	\centering\decoratorexample{height=60mm}
% \end{frame}

\begin{frame}[fragile]
	\frametitle{\insertsubsection{} -- vs Naive}
	\begin{fancycolumns}[widths={50,50}, t]
		\centering \textbf{Object Adapter Pattern}
		\begin{example}{Wrapper}
			\vspace{-3mm}
			\begin{lstlisting}[language=Java, basicstyle=\footnotesize]
public class OrderFormulaWrapper extends FormulaComponent {

public OrderFormulaWrapper(Order order) {
        this.internalOrder = order;
}

@Override
    public int compute() {
        ... // compute based on internal order
    }
}
			\end{lstlisting}
			\vspace{-3mm}
		\end{example}
		\begin{example}{Formula with Orders}
			\vspace{-3mm}
			\begin{lstlisting}[language=Java, basicstyle=\footnotesize]
FormulaComponent formulaRoot = new Sum(
	new OrderFormulaWrapper(new Order(ORDER_ONE, "Ba Sing Se", 3)),
	new Product(new Literal(3), new OrderFormulaWrapper(new Order(ORDER_THREE, "Zaofu", 1))));
			\end{lstlisting}
			\vspace{-3mm}
		\end{example}
		\nextcolumn
		\centering \textbf{Naive}
		\begin{example}{Single Conversion Method}
			\vspace{-3mm}
			\begin{lstlisting}[language=Java, basicstyle=\footnotesize]
public class Order implements WarehouseRequest {...}

private static int getOrderCost(Order order) {
	int sum = 0;
	for (String item : order.orderListWithQuantities.keySet()) {
		sum += CATALOGUE.get(item) * order.orderListWithQuantities.get(item);
	}
	return sum;
}
			\end{lstlisting}
			\vspace{-3mm}
		\end{example}
		\begin{example}{Formula with Orders}
			\vspace{-3mm}
			\begin{lstlisting}[language=Java, basicstyle=\footnotesize]
        FormulaComponent formulaRoot = new Sum(
                new Literal(getOrderCost(new Order(ORDER_ONE, "Ba Sing Se", 3))),
                new Product(new Literal(3), new Literal(getOrderCost(new Order(ORDER_THREE, "Zaofu", 1)))));
		\end{lstlisting}
			\vspace{-3mm}
		\end{example}
	\end{fancycolumns}


\end{frame}


% TODO better composite example? JComponent enough?
%\begin{frame}{Example of the Composite Pattern}
%	\centering\compositeexample{height=60mm}
%\end{frame}

\subsection{Decorator Pattern}
\begin{frame}{\insertsubsection}
	\begin{fancycolumns}
		\begin{definition}{Decorator \mysource{\gof}}
			\setlength\tabcolsep{1mm}
			\begin{tabularx}{\textwidth}{rX}
				intent     & \mycite{Attach additional responsibilities to an object dynamically. Decorators provide a flexible alternative to subclassing for extending functionality.} \\
				aka.       & wrapper (again)                                                                                                                                             \\
				motivation & avoid explosion of static classes when combining all additional behaviors with all applicable classes                                                       \\
				idea       & create decorators and components with the same interface, whereas decorators forward behavior whenever feasible
			\end{tabularx}
		\end{definition}
		\nextcolumn
		\decorator{width=\linewidth}
	\end{fancycolumns}
\end{frame}

\begin{frame}{\insertsubsection{} -- Example}
	\begin{fancycolumns}[widths={35,65}]
		\begin{example}{Example -- Logger Variants}
			\begin{itemize}
				\item Basic logger can be extended to various variants
				\item Decorator pattern allows use to use arbitrary combinations of logger features
			\end{itemize}
		\end{example}
		\nextcolumn
		\begin{figure}[htbp]
			\centering

			\includegraphics[width=\columnwidth]{uml/decoratorexample.pdf}
		\end{figure}
	\end{fancycolumns}
\end{frame}

% \begin{frame}{Example of the Decorator Pattern \mytitlesource{\href{https://docs.oracle.com/en/java/javase/21/docs/api/java.base/java/io/FilterInputStream.html}{Java 21}}}
% 	\centering\decoratorexample{height=60mm}
% \end{frame}

\begin{frame}[fragile]
	\frametitle{\insertsubsection -- vs Naive}
	\begin{fancycolumns}[widths={50,50}, t]
		\centering \textbf{Decorator Pattern}
		\begin{example}{Class}
			\vspace{-3mm}
			\begin{lstlisting}[language=Java, basicstyle=\small]
public CauseLogger(AbstractLogger wrappedLogger) {
	super(wrappedLogger);
	this.wrappedLogger = wrappedLogger;
}
// Instantiate a variant
AbstractLogger motivationalCauseLogger = new MotivationalLogger(new CauseLogger(new BaseLogger()));
			\end{lstlisting}
			\vspace{-3mm}
		\end{example}
		\begin{example}{Creating Messages}
			\vspace{-3mm}
			\begin{lstlisting}[language=Java, basicstyle=\small]
@Override
public String createMessage(Exception e) {
	return wrappedLogger.createMessage(e) + " " + e.getStackTrace().toString();
}
			\end{lstlisting}
			\vspace{-3mm}
		\end{example}
		\nextcolumn
		\centering \textbf{Naive}
		\begin{example}{Class}
			\vspace{-3mm}
			\begin{lstlisting}[language=Java, basicstyle=\footnotesize]
public Logger(boolean beMotivational, boolean printCause, boolean printTrace) {
	this.beMotivational = beMotivational;
	this.printCause = printCause;
	this.printTrace = printTrace;
}
			\end{lstlisting}
			\vspace{-3mm}
		\end{example}
		\begin{example}{Creating Messages}
			\vspace{-3mm}
			\begin{lstlisting}[language=Java, basicstyle=\footnotesize]
public String createMessage(Exception e) {
	String message = e.getMessage();
	if (beMotivational) {
		message = "Come on. You got this! " + message;
	}
	if (printCause) {
		message = message + " " + e.getCause() == null ? "Root cause" : e.getCause().toString();
	}
	if (printTrace) {
		message = message + " " + e.getStackTrace().toString();
	}
	return message;
}
		\end{lstlisting}
			\vspace{-3mm}
		\end{example}
	\end{fancycolumns}
\end{frame}

\againframe{structuralpatterns}


%\begin{frame}[t]{\insertsubsection{} \mytitlesource{\umlrefman\mypages{91--92,198--199,387--388}; \umluserguide\mypages{318--323}; \umlspec\mypages{217--218}}} % TODO copied from SE1. reuse?
%	\alt<2->{\profcalculatorcollaboration{width=\linewidth,page=2}}{\profcalculatorcollaboration{width=\linewidth,page=6}}
%	
%	\vspace{-75mm}
%	\begin{fancycolumns}[widths={65},animation=none]
%		\nextcolumn
%		\mynotetight{Object Adapter Pattern}{\objectadapter{width=\linewidth}}
%	\end{fancycolumns}
%\end{frame}

% TODO create a separate video for each design pattern. would result in shorter, more manageable videos and more flexible usage also in other courses.
% TODO idea: much more modular videos that we can mix and match between numerous lectures. would require to not write the lecture on each slide. could result in a network of material on youtube, in which videos link to each other rather flexible and we could also remove or add parts without breaking the slide numbering.
% TODO open question: shall we have a consecutive running example or should each design pattern rather start from the same base implementation? would enable feature branches and their merging into releases with several design patterns. could also be nice to illustrate VCS and release management.

% TODO missing on slides!
%\subsection{Welche der folgenden Aussagen stimmen?}
%\begin{itemize}
%    \richtig Ein Entwurfsmuster ist eine generische, bewährte Lösung für ein wiederkehrendes Entwurfsproblem
%    \falsch Entwurfsmuster sind nur Jargon, Regeln, und Programmiertricks
%    \falsch Die Anwendung von Entwurfsmustern erzeugt eine komplette Systemarchitektur
%    \richtig Entwurfsmuster werden in Strukturmuster, Erzeugungsmuster und Verhaltensmuster unterteilt
%\end{itemize}

% design patterns, cohesion, Kopplung, ...
% TODO sequence diagrams: better explanation needed why activities are always enclosed in the synchronous case
% Mehrfachvererbung: \mycite{A class that has exactly one parent is said to use single inheritance; a class with more than one parent is said to use multiple inheritance.} \umluserguide
% TODO live coding on design patterns (43min)
% 32:37..35:45 + 36:10..37:15 + 37:36..38:33 complex expressions (composite pattern and object adapter 8min)
% 38:55 function (observer pattern 9min) - replace slide at 43:02 and 45:25 - implement observer pattern completely
% 46:08 delete
% 46:12 logging (singleton pattern 10min) - replace slide at 56:25
% 56:46 font size (abstract factory 10min) - replace slide at 1:05:20 - implement abstract factory
% 1:06:50 brackets (decorator pattern 6min) - replace slide at 1:10:50 - implement decorator pattern
% 1:12:48 coverage
% 1:14:25 delete
% TODO add UML notation for design patterns (i.e., collaborations)

