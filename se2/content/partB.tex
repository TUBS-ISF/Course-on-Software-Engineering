...
\faq{
	...
	\item Why are not all instructions equally fast? Give an example!
	\item Why are certain high-level language features (e.g., array access, loops, method calls, objects, garbage collection) slow when translated?
	\item Shall developers completely omit slow language features?
	\item What is the connection between compiler optimizations, smells, and refactorings?
	\item How to avoid problems resulting from garbage collection?
...
\faq{
	...
	\item What is the Ada programming language (used for)?
	\item What is SPARK?
	\item Why are language features added/removed compared to Ada?
	\item Which tools does SPARK offer for development and verification?
	\item How is design by contract implemented in SPARK?
	\item How can we specify and analyze code contracts in SPARK?
}
