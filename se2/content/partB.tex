...
\ifuniversity{tubs}{\date{May 17, 2026}}
...
\lessonslearned{
	...
	\item Analysis results of compilers and interpreters
	\item Error types
...
\lessonslearned{
	...
	\item Main concepts for scanning, parsing, and code generation
	...
	\begin{enumerate}
		...
		\item Why is the pipe-and-filter architecture used for compilers?
		\item What properties are desirable for intermediate languages?
	\end{enumerate}
}
...
\lessonslearned{
	...
	\item What is the impact of compiler optimizations on software design and development?
	\item How to manage memory? What to consider when choosing approach for memory management?
	\item Next: How to analyze programs during runtime?
}{
	\item \dragonbook
}{
	\begin{itemize}
		\item Which of the presented optimizations make sense to apply as a refactoring? 
		\item Which should only be applied by the compiler?
	\end{itemize}
}

\faq{
	\item Why is the choice of programming language crucial in SE?
	\item What are (dis)advantages of using many programming languages?
	\item Why is compilation crucial for high-level programming languages?
	\item What are commonalities and differences between compilation and interpretation? Illustrate inputs+outputs!
	\item What are errors and warnings in the context of compilation?
	\item What is the difference between lexical, syntax, type, runtime and logical errors? Give an example!
	\item How to identify errors of type X?
	...
	\item What is the Chomsky hierarchy? What is the impact on compilers?
	\item What are the common stages in a compiler?
	\item Which concept are used to scan the source code?
	\item Which concept is used to parse the token stream?
	\item How to generate code from the syntax tree?
	\item What are advantages of intermediate languages? 
}{
	\item What are compiler optimizations (good for)?
	...
	\item Why are certain high-level language features (e.g., loops, method calls, certain arithmetic operators) slow when translated?
	\item What is the connection between compiler optimizations, smells, and refactorings?
...
\input{content/06a-testingrecap}
\lessonslearned{
...
\input{content/06b-unitintegration}
\lessonslearned{
...
\input{content/06c-systemacceptance}
\lessonslearned{
...
\faq{
	...
	\item How is the quality of a test suite measured?
	\item What are flaky tests and how do we handle them?
	\item How do we test component interactions?
	\item What is the difference between top-down, buttom-up and big-bang integration testing? What are (dis)advantages?
	\item What different kinds of code coverage exist?
	...
	\item What are (dis)advantages of manual and automated tests?
	\item What are stages of manual testing?
	...
	% TODO Raphael, too unspecific? \item What happens before a software product is accepted by the customer?
...
\input{content/07b-designbycontract}
\lessonslearned{
	...
	\item \designbycontractjml{} --- overview and considerations for design by contract with JML
}{
	\begin{itemize}
		\item Form groups of 2--4 students
		\item For every group, you need a laptop to work on the exercise in lecture part 07c
	\end{itemize}
}

\section{Exercise -- Design by Contract with JML}
\input{content/07c-jml}
% \section{Design by Contract with SPARK}
% \input{content/07c-spark}
\lessonslearned{
	\item Design by contract with JML
	\item Live demo with examples
	\item Next: How is quality assurance embedded in continuous integration?
}{
	\item \href{https://www.openjml.org/}{OpenJML Documentation}
}{
	Continue the \href{https://www.openjml.org/tutorial/}{OpenJML tutorial}
	
	\vspace{0.2cm}
	
	\centering\fancyqr{color=black,height=40mm}{https://www.openjml.org/tutorial/}
}

\faq{
	\item What is the Ariane 5 software failure?
	\item How is it related to software engineering?
	\item How could it have been prevented?
	\item What can we learn from it?
	\item How can developers document their assumptions about the code?
	\item What are (dis)advantages of code documentation, defensive programming, runtime assertions, unit testing?
	\item Given a pair of techniques, what do they have in common and where do they differ?
}{
	\item What is (the motivation for) design by contract?
	\item What is (the motivation for) behavioral interface specification language(s)?
	\item What are examples for behavioral interface speciﬁcation languages?
	\item What is the Java Modeling Language (used for)?
	\item How to automatically check JML specifications?
	\item How does the JML compare to other assumption documentation techniques?
}{
	\item How to model assumptions with JML?
	\item Translate a given assumption into JML!
	\item Understand a given JML specification!
}
...
\ifuniversity{tubs}{\date{June 15, 2026}}

\author{Raphael Dunkel}
\lecture{Configuration Management}{configuration}

\section{Configuration Management}
\input{content/08a-configmanagement}
\lessonslearned{
...
\lessonslearned{
	...
	Learn more about the presented concepts from popular tools:
	\begin{itemize}
		\item CI/CD (GitHub Actions): \url{https://docs.github.com/en/actions}
		\item Containerization (Docker): \url{https://docs.docker.com/get-started/}
		\item Test-Driven Development (No tool, but Blog Post): \url{https://newsletter.kentbeck.com/p/canon-tdd}
	\end{itemize}
}
...
\faq{
	...
	\item What is a version control system (good for)?
	\item What are approaches to version control and how do they differ?
	\item What is system building, change management, release management?
	...
	\item What is the difference between merging, patching, cherry-picking, and rebasing?
	...
	\item What are (dis)advantages of software containers?
