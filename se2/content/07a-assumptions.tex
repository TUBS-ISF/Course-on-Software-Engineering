...
\begin{frame}[fragile]
	\frametitle{The Need to Document Assumptions -- \insertsubsection{}}
	\begin{example}{Library Method}
		\begin{lstlisting}[style=java]
public static int getStudentRecord(int semester, List<Course> finishedCourses) {// ...?}
		\end{lstlisting}
	\end{example}
	\pause
		\begin{example}{A Look Inside}
			\begin{lstlisting}[style=java,basicstyle=\small]
			public static Record getStudentRecord(int semester, List<Course> finishedCourses) {
				int overallCredits = 0;
				double averageGrade = 1.0;
				int creditsPerSemester = 0;
				for (Course course : finishedCourses) {
					overallCredits += course.getCredits();
					averageGrade += course.getGrade() * course.getCredits();
				}
				averageGrade /= overallCredits;
				creditsPerSemester = overallCredits / semester;
				return new Record(overallCredits, averageGrade, creditsPerSemester);
			}
			\end{lstlisting}
		\end{example}
\end{frame}
	...
	\begin{fancycolumns}[t]
		...
		\begin{definition}{\insertsubsection}
			\begin{itemize}
				...
				\item[+] assumptions easily accessible during coding
	...
	\begin{fancycolumns}
		...
		\begin{definition}{\insertsubsection}
			\begin{itemize}
				...
				\item[+] unambiguous, automated checks
				\item[+] intention is more explicit than in defensive programming 
				\item[--] not accessible for callers (esp. with overriding)
