...
\begin{frame}{Refactoring \mytitlesource{\ludewiglichter}}
	...
	\begin{fancycolumns}
		...
		\begin{example}{Refactorings}
			...
			\end{itemize}
		\end{example}
		\begin{example}{Smells and Refactorings (Examples SE2-2025)}
...
\begin{frame}
	...
	\begin{figure}[htbp]
		\centering
		\begin{tikzpicture}[node distance=2cm and 1.2cm]
			...
			\node[ioblock, right = 2cm of modularization] (updatedprogram) {Updated Program};
			\draw[->] (modularization) -- node[sloped, above, midway] {\footnotesize Forward Engineering} (updatedprogram);
			\node[ioblock, right = 2cm of updatedprogram] (newdata) {Updated Data};
...
\begin{frame}
	...
	\begin{fancycolumns}[widths={50,50}]
		...
		\begin{definition}{Advantages}
			\begin{itemize}
				\item Simple: No planning and coordination required for parallel operation
